...
Much like we did in Whitacbc of the motors unit, later we will show you different (better) ways to use buttons.
...
def main():
    ...
    new_led_colors = [ev3.Leds.GREEN, ev3.Leds.RED, ev3.Leds.AMBER, ev3.Leds.BLACK]
    while True:
        if btn.up:
            print('up')
            ev3.Leds.all_off()
        ...
        if btn.right:
            ...
            ev3.Leds.set_color(ev3.Leds.LEFT, ev3.Leds.BLACK)
        if btn.down:
            ev3.Leds.set_color(ev3.Leds.RIGHT, new_led_colors[current_color_index])
            ev3.Leds.set_color(ev3.Leds.LEFT, new_led_colors[current_color_index])
