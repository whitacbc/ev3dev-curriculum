...
Authors: David Fisher and Brian Whitacre.
...
def main():
    ...
    forward_button['command'] = lambda: some_callback2(mqtt_client, left_speed_entry, right_speed_entry)
    root.bind('<Left>', lambda event: some_callback2(mqtt_client, left_speed_entry, right_speed_entry))

    stop_button = ttk.Button(main_frame, text="Stop")
    stop_button.grid(row=3, column=1)
    # stop_button and '<space>' key (note, does not need left_speed_entry, right_speed_entry)
    forward_button['command'] = lambda: some_callback3(mqtt_client, left_speed_entry, right_speed_entry)
    root.bind('<space>', lambda event: some_callback3(mqtt_client, left_speed_entry, right_speed_entry))

    right_button = ttk.Button(main_frame, text="Right")
    right_button.grid(row=3, column=2)
    # right_button and '<Right>' key
    forward_button['command'] = lambda: some_callback4(mqtt_client, left_speed_entry, right_speed_entry)
    root.bind('<Right>', lambda event: some_callback4(mqtt_client, left_speed_entry, right_speed_entry))

    back_button = ttk.Button(main_frame, text="Back")
    back_button.grid(row=4, column=1)
    # back_button and '<Down>' key
    forward_button['command'] = lambda: some_callback5(mqtt_client, left_speed_entry, right_speed_entry)
    root.bind('<Down>', lambda event: some_callback5(mqtt_client, left_speed_entry, right_speed_entry))
...
def some_callback1(mqtt_client, left_speed_entry, right_speed_entry):
    mqtt_client.send_message('speed',[left_speed_entry,right_speed_entry])

def some_callback2(mqtt_client, left_speed_entry, right_speed_entry):
    mqtt_client.send_message('speed',[left_speed_entry,right_speed_entry])

def some_callback3(mqtt_client, left_speed_entry, right_speed_entry):
    mqtt_client.send_message('speed',[left_speed_entry,right_speed_entry])

def some_callback4(mqtt_client, left_speed_entry, right_speed_entry):
    mqtt_client.send_message('speed',[left_speed_entry,right_speed_entry])

def some_callback5(mqtt_client, left_speed_entry, right_speed_entry):
    mqtt_client.send_message('speed',[left_speed_entry,right_speed_entry])
